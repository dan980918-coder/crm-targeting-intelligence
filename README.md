...
> **핵심 결과 요약**: 구매성향 모델(Model B)은 최근성 규칙보다 접촉
> 인원을 최대 50.87% 줄이면서 같은 포착률(Recall)을 낸다. 구매 비활성
> 모델(Model A)은 통계적으로는 규칙보다 낫지만(AUC 0.754 vs 0.669)
> 실무 효과는 작다 — 라벨 기저율이 이미 90%가 넘어서다. 라이프사이클
> 임계값과 우측 검열 처리는 전부 데이터 기반으로 설계했다. 상세는
> "11. 핵심 결과", 세운 가설과 검증 과정은
> [`docs/hypotheses.md`](docs/hypotheses.md) 참고.
...
**데이터는 2022-06-23 \~ 2022-12-08, 6개월(167일)치다.** 최신 고객 행동이
아니라 **2022년 하반기의 한 스냅샷**이다 (근거: `PROJECT_GUIDELINES.md` 2번·10번 섹션의
"[2026-08-05 정책 갱신]" 인라인 표기, `docs/limitations.md` 1번 항목).
...
**Model A는 AUC로 평가해야 한다.** 라벨을 14/28일 대신 60일로 늘려
직접 검증해봤다 — 라벨이 희소해질수록(양성률 94.65%→83.95%) Lift@10%는
예측대로 올랐지만(1.038→1.105), AUC는 오히려 떨어졌다(0.743→0.711).
Lift가 오른 게 모델이 좋아졌다는 뜻이 아니었다는 얘기다. 상세:
[`docs/hypotheses.md`](docs/hypotheses.md) 8번.
...
접촉 인원을 얼마나 줄일 수 있는지 봤다. Model A는 AUC(0.754)로는 규칙보다
뚜렷이 우수하지만, 아래 절감률 숫자는 그 우수함을 온전히 반영하지 못한다
— 라벨(90%+가 다수 클래스) 구조상 Lift로 표현되는 접촉 효율 개선 여지가
원천적으로 작기 때문이다(자세한 검증: 위 7번, `docs/hypotheses.md` 8번):

| 모델 | 절감률 (5\~30% 접촉 구간) |
|---|---|
| Model A (구매 비활성) | 0.58% \~ 1.64% (라벨 구조상 제한적 — AUC는 우수) |
...
- 구매 비활성 위험 예측은 **AUC** 기준으로 규칙보다 낫다(0.754 vs 0.669).
  다만 라벨 기저율이 이미 90%가 넘어서, Lift로 드러나는 접촉 절감 효과는
  크지 않다 — 라벨을 60일로 늘려 직접 검증해보니 Lift는 오르고 AUC는
  떨어졌다. 이 모델은 Lift가 아니라 AUC로 평가하는 게 맞다는 뜻이다
