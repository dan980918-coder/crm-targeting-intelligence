...
metric(c1, "explore", "탐색 단계", format_count(group.get("neutral", 0)),
       ...
       help="탐색_고객 + 장바구니_고객")
metric(c2, "active", "구매 활성/성장", format_count(group.get("positive", 0)),
       sub=f"전체의 {group.get('positive', 0)/df['n'].sum()*100:.1f}%", tone="positive",
       help="첫_관측_구매_고객 + 활성_구매_고객 + 복귀_고객")
metric(c3, "declining", "구매 감소/위험", format_count(group.get("warning", 0)),
       sub=f"전체의 {group.get('warning', 0)/df['n'].sum()*100:.1f}%", tone="warning",
       help="구매_비활성_위험_고객 + 구매_감소_고객")
